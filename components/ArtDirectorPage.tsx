...
const ArtDirectorPage: React.FC<ArtDirectorPageProps> = ({ onBack, initialContext }) => {
  ...
  const exportFullCampaign = async () => {
    ...
    try {
      ...
      // Page 1: Overview
      doc.setFillColor(11, 15, 20); // Dark background for header
      ...
      doc.text('BRANDWIZ CAMPAIGN REPORT', margin, y);
      
      y = 50;
      doc.setTextColor(0, 0, 0);
      addText(`Campaign: ${initialContext.strategy?.planName || 'Master Campaign'}`, 18, 'bold');
      addText(`Generated on: ${new Date().toLocaleDateString()}`, 10, 'italic', '#666666');
      y += 10;

      addText('1. EXECUTIVE SUMMARY', 14, 'bold', '#00d8ff');
      addText(`Business Niche: ${initialContext.niche}`);
      addText(`Location: ${initialContext.location}`);
      addText(`Current Trend: ${initialContext.trend}`);
      addText(`Primary Goal: ${initialContext.primaryGoal || initialContext.goal}`);
      y += 10;

      // Page 2: Strategy
      if (initialContext.strategy) {
        doc.addPage();
        y = 20;
        addText('2. STRATEGIC ROADMAP', 14, 'bold', '#00d8ff');
        initialContext.strategy.phases.forEach((phase) => {
          addText(`Day ${phase.day}: ${phase.task}`, 12, 'bold');
          addText(`Objective: ${phase.objective}`, 10, 'normal', '#444444');
          y += 5;
        });
      }

      // Page 3: Copywriting
      if (initialContext.copy) {
        doc.addPage();
        y = 20;
        addText('3. COPYWRITING ASSETS', 14, 'bold', '#00d8ff');
        
        addText('Taglines:', 12, 'bold');
        initialContext.copy.taglines.forEach(tag => addText(`- ${tag}`, 11));
        y += 5;

        addText('Platform Variants:', 12, 'bold');
        initialContext.copy.variants.forEach(variant => {
          addText(`${variant.platform}: ${variant.headline}`, 11, 'bold');
          addText(variant.body, 10);
          addText(`CTA: ${variant.cta}`, 10, 'italic', '#00d8ff');
          y += 5;
        });
      }

      // Page 4: Art Direction
      doc.addPage();
      y = 20;
      addText('4. VISUAL DIRECTION', 14, 'bold', '#00d8ff');
      addText(`Vibe: ${visualBrief.vibe}`);
      addText('Color Palette:', 12, 'bold');
      visualBrief.primaryPalette.forEach(color => {
        doc.setFillColor(color);
        doc.rect(margin, y, 10, 10, 'F');
        doc.setTextColor(0, 0, 0);
        doc.text(color, margin + 15, y + 7);
        y += 15;
      });
      y += 5;

      addText('Psychological Intent:', 12, 'bold');
      visualBrief.psychologicalIntent.forEach(item => {
        addText(`${item.colorName}: ${item.intent}`, 10);
      });
      ...
      if (generatedImages.length > 0) {
        doc.addPage();
        y = 20;
        addText('5. GENERATED VISUAL ASSETS', 14, 'bold', '#00d8ff');
        ...
        for (let i = 0; i < generatedImages.length; i++) {
          ...
          // 16:9 aspect ratio: w=170, h=95.6
          doc.addImage(img, 'PNG', margin, y, 170, 95.6);
          y += 105;
          addText(`Variation ${String.fromCharCode(65 + i)}: ${variations[i]?.name || ''}`, 10, 'italic');
          y += 5;
        }
      }

      doc.save(`${initialContext.strategy?.planName || 'BrandWiz_Campaign'}.pdf`);
    } catch (error) {
        ...
        <footer className="fixed bottom-0 left-0 right-0 p-6 bg-[#0b0f14]/80 backdrop-blur-xl border-t border-gray-800/60 z-30">
          <div className="max-w-7xl mx-auto flex justify-between items-center">
            ...
            <button 
              ...
              className={`px-10 py-4 ${isExporting ? 'bg-gray-700 cursor-not-allowed' : 'bg-emerald-500 hover:bg-emerald-600'} text-black font-extrabold rounded-2xl transition-all shadow-xl shadow-emerald-500/20 flex items-center gap-3 active:scale-95 group`}
            >
              {isExporting ? 'Exporting...' : 'Export Full Campaign'}
              <svg className={`w-6 h-6 ${isExporting ? '' : 'group-hover:translate-x-1'} transition-transform`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
            </button>
          </div>
        </footer>
      )}
